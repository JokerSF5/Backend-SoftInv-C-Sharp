...
namespace SoftInv.DAOImpl
{
    public abstract class DAOImplBase
    {
        ...
        protected void ColocarSQLenComando(string sql, System.Data.CommandType tipo = System.Data.CommandType.Text)
        {
        ...
        protected int EjecutarModificacionEnBD()
        {
            if (this.mostrarSentenciaSQL)
            {
                Console.WriteLine(this.comando.CommandText);
            }
        ...
        protected void EjecutarProcedimientoAlmacenado(string sql, bool conTransaccion)
        {
            Action<object> incluirValorDeParametros = null;
            object parametros = null;
            EjecutarProcedimientoAlmacenado(sql, incluirValorDeParametros, parametros, conTransaccion);
        }

        protected void EjecutarProcedimientoAlmacenado(string sql, Action<object> incluirValorDeParametros, object parametros, bool conTransaccion)
        {
            try
            {
                if (conTransaccion)
                ...
                // Cambiado aquí:
                ColocarSQLenComando(sql, System.Data.CommandType.StoredProcedure);
            ...
            catch (SqlException ex)
            {
                if (conTransaccion)
                {
                    ...
                    {
                        Console.Error.WriteLine("Error en rollback: " + ex1.Message);
                    }
                }
                Console.Error.WriteLine("Error en procedimiento: " + ex.Message);
            }
            finally
            {
                try
                ...
                {
                    Console.Error.WriteLine("Error al cerrar conexión: " + ex.Message);
                }
            }
        }
